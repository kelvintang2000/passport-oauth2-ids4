 ...
 *   - `userinfoURL`       URL used to obtain user information
...
function OAuth2Strategy(options, verify) {
  ...
  if (!options.userinfoURL) { throw new TypeError('OAuth2Strategy requires a userinfoURL option'); }
  ...
  this._oauth2 = new OAuth2(options.clientID,  options.clientSecret,
      '', options.authorizationURL, options.tokenURL, options.userinfoURL, options.customHeaders);
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  var meta = {
	...
	userinfoURL: this._oauth2._userinfoUrl,
...
OAuth2Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  function callback(error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      //console.log("first name: " + info.family_name);
      ...
      return done(null,user,info=user);
    }
    else
      return done(null, {});
  }
